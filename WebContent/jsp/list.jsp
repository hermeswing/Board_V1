...
<div id="wrap">
	<table>
		...
		<thead>
	 	...
	 	<tbody>
			...
			<tr>
				<td align="center">
					<antop:bunho page="${page}" total="${total}" rowIdx="${stat.index}" rowSize="${rowSize}" />
				</td>
				<td><c:if test="${fn:length(b.attachs) > 0}"><img src="images/disk.png" class="icon" title="파일첨부 ${fn:length(b.attachs)}개"></c:if></td>
				<td><c:if test="${b.security == true}"><img src="images/lock.png" class="icon" title="비밀글"></c:if></td>
				<td>
					<c:if test="${b.dept > 0}">
					...
					<c:if test="${b.dept > 0}"><img src="images/reply.png" class="icon"></c:if>
					<a href="view.do?bid=${b.boardId}&page=${page}&search=${search}">
						<c:out value="${b.subject}" escapeXml="true" />
					</a>
				</td>
	...
	<form id="form" name="list.do" method="get">
		<div class="buttons">
